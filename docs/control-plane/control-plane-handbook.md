...
- 边界影响变更：触及 scope/contract/验收门槛/治理职责，需经充分讨论（会议、Issue 讨论、口头对齐等）确认影响面后走 OpenSpec。
- 紧急止损变更：允许最小化止血，事后补齐讨论、审计链路和必要的 OpenSpec 变更。
...
- 解冻条件：经充分讨论确认影响面，重大变更需通过 OpenSpec 审核，且回滚口径明确，才允许解冻改写。
